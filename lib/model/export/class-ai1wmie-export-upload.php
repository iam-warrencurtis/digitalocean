...
class Ai1wmie_Export_Upload {
	...
	public static function execute( $params, Ai1wmie_DigitalOcean_Client $digitalocean = null ) {
		...
		if ( ! isset( $params['upload_id'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Upload ID is not specified.', AI1WMIE_PLUGIN_NAME ) );
		}
		...
		if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
			...
			try {
				...
				$file_chunk_etag = $digitalocean->upload_file_chunk( $file_chunk_data, $file_path, $params['upload_id'], $params['bucket_name'], $params['region_name'], $params['file_chunk_number'] );
			...
			if ( defined( 'WP_CLI' ) ) {
				WP_CLI::log(
					sprintf(
						__( 'Uploading %s (%s) [%d%% complete]', AI1WMIE_PLUGIN_NAME ),
						$name,
						$size,
						$progress
					)
				...
				Ai1wm_Status::info(
					sprintf(
						__(
							'<i class="ai1wmie-icon-digitalocean"></i> ' .
			...
			$digitalocean->upload_complete( $file_chunks, $file_path, $params['upload_id'], $params['bucket_name'], $params['region_name'] );

			// Set last backup date
			update_option( 'ai1wmie_digitalocean_timestamp', time() );
