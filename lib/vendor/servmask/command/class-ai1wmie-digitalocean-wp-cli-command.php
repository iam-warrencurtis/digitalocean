...
if ( defined( 'WP_CLI' ) ) {
	/**
	 * All-in-One WP Migration DigitalOcean Spaces
	 *
	 * ## EXAMPLES
	 *
	 *     # Backup site to DigitalOcean Spaces.
	 *     $ wp ai1wm digitalocean backup
	 *     Success: Backup complete.
	 *
	 *     # List existing DigitalOcean backups.
	 *     $ wp ai1wm digitalocean list-backups
	 *     +--------------------------------------+--------------+----------+
	 ...
	 *     +--------------------------------------+--------------+----------+
	 *
	 *     # Restore website from DigitalOcean backups.
	 *     $ wp ai1wm digitalocean restore wordpress-20181123-071143-621.wpress --yes
	 *     Success: Restore complete.
	 */
	class Ai1wmie_DigitalOcean_WP_CLI_Command extends WP_CLI_Command {
		public function __construct() {
			if ( ! defined( 'AI1WM_PLUGIN_NAME' ) ) {
				WP_CLI::error_multi_line( array(
					__( 'DigitalOcean Spaces Extension requires All-in-One WP Migration plugin to be activated. ', AI1WMIE_PLUGIN_NAME ),
					__( 'You can get a copy of it here: https://wordpress.org/plugins/all-in-one-wp-migration/', AI1WMIE_PLUGIN_NAME ),
				) );
				exit;
			}
			...
			if ( is_multisite() && ! defined( 'AI1WMME_PLUGIN_NAME' ) ) {
				WP_CLI::error_multi_line( array(
					__( 'WordPress Multisite is supported via our All in One WP Migration Multisite Extension.', AI1WMIE_PLUGIN_NAME ),
					__( 'You can get a copy of it here: https://servmask.com/products/multisite-extension', AI1WMIE_PLUGIN_NAME ),
				) );
				exit;
			}
			...
			if ( ! is_dir( AI1WM_STORAGE_PATH ) ) {
				if ( ! mkdir( AI1WM_STORAGE_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'All in One WP Migration is not able to create <strong>%s</strong> folder.', AI1WMIE_PLUGIN_NAME ), AI1WM_STORAGE_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All in One WP Migration plugin to function properly.', AI1WMIE_PLUGIN_NAME ),
					) );
					exit;
				}
			}
			...
			if ( ! is_dir( AI1WM_BACKUPS_PATH ) ) {
				if ( ! mkdir( AI1WM_BACKUPS_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'All in One WP Migration is not able to create <strong>%s</strong> folder.', AI1WMIE_PLUGIN_NAME ), AI1WM_BACKUPS_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All in One WP Migration plugin to function properly.', AI1WMIE_PLUGIN_NAME ),
					) );
					exit;
				}
			...
			if ( ! get_option( 'ai1wmie_digitalocean_access_key', false ) || ! get_option( 'ai1wmie_digitalocean_secret_key', false ) ) {
				WP_CLI::error_multi_line( array(
					__( 'In order to use All in One WP Migration DigitalOcean Spaces extension you need to configure it first.', AI1WMIE_PLUGIN_NAME ),
					__( 'Please navigate to WP Admin > All-in-One WP Migration > DigitalOcean Spaces Settings and specify your DO access and secret keys.', AI1WMIE_PLUGIN_NAME ),
				) );
				exit;
			}
		}
		...
		/**
		 * Creates a new backup and uploads to DigitalOcean Spaces.
		 *
		 * ## OPTIONS
		 *
		 * [--sites]
		 ...
		 * ## EXAMPLES
		 *
		 * $ wp ai1wm digitalocean backup --replace "wp" "WordPress"
		 * Backup in progress...
		 * Uploading wordpress-20190509-135835-448.wpress (33 MB) [60% complete]
		 * Uploading wordpress-20190509-135835-448.wpress (33 MB) [100% complete]
		 * Success: Backup complete.
		 * Backup file: wordpress-20190509-135835-448.wpress
		 * Backup location: [region: sfo2] [bucket: servmask] [path: wordpress-20190509-135835-448.wpress]
		 *
		 * @subcommand backup
		 */
		public function backup( $args = array(), $assoc_args = array() ) {
			...
			WP_CLI::success( __( 'Backup complete.', AI1WMIE_PLUGIN_NAME ) );

			WP_CLI::log( sprintf( __( 'Backup file: %s', AI1WMIE_PLUGIN_NAME ), ai1wm_archive_name( $params ) ) );
			WP_CLI::log( sprintf( __( 'Backup location: %s', AI1WMIE_PLUGIN_NAME ), $this->get_backup_location( $params ) ) );
		}

		/**
		 * Get a list of DigitalOcean backup files.
		 *
		 * ## OPTIONS
		 *
		 * [--folder-path=<path>]
		 * : List backups in a specific DigitalOcen folder
		 *
		 * ## EXAMPLES
		 *
		 * $ wp ai1wm digitalocean list-backups
		 ...
		 * +------------------------------------------------+--------------+-----------+
		 *
		 * $ wp ai1wm digitalocean list-backups --folder-path=/backups/daily
		 * +------------------------------------------------+--------------+-----------+
		 * | Backup name                                    | Date created | Size      |
		 * +------------------------------------------------+--------------+-----------+
		 * | migration-wp-20170908-152313-435.wpress        | 4 days ago   | 536.77 MB |
		...
		public function list_backups( $args = array(), $assoc_args = array() ) {
			...
			$backups->setHeaders( array(
				'name' => __( 'Backup name', AI1WMIE_PLUGIN_NAME ),
				'date' => __( 'Date created', AI1WMIE_PLUGIN_NAME ),
				'size' => __( 'Size', AI1WMIE_PLUGIN_NAME ),
			) );
			...
			foreach ( $items as $item ) {
				if ( pathinfo( $item['name'], PATHINFO_EXTENSION ) === 'wpress' ) {
					$backups->addRow( array(
						'name' => $item['name'],
						'date' => sprintf( __( '%s ago', AI1WMIE_PLUGIN_NAME ), human_time_diff( $item['date'] ) ),
		...
		/**
		 * Restores a backup from DigitalOcean Spaces.
		 *
		 * ## OPTIONS
		 *
		 * <file>
		 ...
		 *
		 * [--folder-path=<path>]
		 * : Download a backup from a specific DigitalOcean folder inside a bucket
		 *
		 * [--yes]
		 ...
		 * ## EXAMPLES
		 *
		 * $ wp ai1wm digitalocean restore migration-wp-20170913-095743-931.wpress
		 * Restore in progress...
		 * Restore complete.
		 *
		 * $ wp ai1wm digitalocean restore migration-wp-20170913-095743-931.wpress --folder-path=/backups/daily
		 * @subcommand restore
		 */
		public function restore( $args = array(), $assoc_args = array() ) {
			if ( ! isset( $args[0] ) ) {
				WP_CLI::error_multi_line( array(
					__( 'A backup name must be provided in order to proceed with the restore process.', AI1WMIE_PLUGIN_NAME ),
					__( 'Example: wp ai1wm digitalocean restore migration-wp-20170913-095743-931.wpress', AI1WMIE_PLUGIN_NAME ),
				) );
				exit;
			}
			...
			if ( is_null( $file ) ) {
				WP_CLI::error_multi_line( array(
					__( "The backup file could not be located in $folder_path folder.", AI1WMIE_PLUGIN_NAME ),
					__( 'To list available backups use: wp ai1wm digitalocean list-backups', AI1WMIE_PLUGIN_NAME ),
				) );
				exit;
			}
		...
		protected function get_folder_path( $assoc_args ) {
			...
			$folder_path = get_option( 'ai1wmie_digitalocean_folder_name', '' );
